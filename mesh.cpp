...
MeshData readMeshData(std::ifstream& mesh_file)
{
    MeshData mesh_data;
    ...
    mesh_data.m_vertex_data.resize(vertex_count);
    mesh_file.read(reinterpret_cast<char*>(m_vertex_data.data()), vertex_count * sizeof(VertexDefault));
...
void MeshManager::loadMeshData(Renderer& renderer, std::string_view mesh_filename)
{
    ...
    if(bone_count != 0)
    {
        ...
        auto mesh_data = readMeshData(mesh_file);
        m_animated_mesh_data.emplace(std::move(mesh_data), std::move(anim_mesh_data));
    }
    else
    {
        m_mesh_data.emplace(readMeshData(mesh_file));
    }
