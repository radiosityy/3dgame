...
layout(location = 3) in vec3 world_pos_in;
layout(location = 4) flat in uvec2 tex_ids;
layout(location = 5) in float view_z_in;
...
layout(set = 0, binding = DIR_SM_BINDING) uniform sampler2DArray dir_shadow_maps[dir_shadow_map_count];
...
void main()
{
    ...
    else
    {
        ...
        const vec3 T = normalize(tan_in - dot(tan_in, norm_in) * norm_in);

        mat3x3 TBN = mat3x3(T, cross(norm_in, T), norm_in);
        N = TBN * N;
    }
    ...
    for(uint i = 0; i < common_buf.dir_light_count; i++)
    {
        ...
        else
        {
            ...
            for(uint sm_id = 0; sm_id < dir_light_buf.lights[i].shadow_map_count; sm_id++)
            {
                ...
                {
                    const vec3 shadow_map_texcoords = vec3(dir_shadow_map_buf.data[dir_light_buf.lights[i].shadow_map_id * MAX_DIR_SHADOW_MAP_PARTITIONS + sm_id].tex_P * vec4(world_pos_in, 1.0f));
                    vec2 t = fract(shadow_map_texcoords.xy * vec2(dir_light_buf.lights[i].shadow_map_res_x, dir_light_buf.lights[i].shadow_map_res_y));
                    ...
                    for(int offset_y = -2; offset_y < 2; offset_y++)
                    {
                        for(int offset_x = -2; offset_x < 2; offset_x++)
                        {
                            ...
                            const float dx0 = float(offset_x) / dir_light_buf.lights[i].shadow_map_res_x;
                            const float dx1 = float(offset_x + 1) / dir_light_buf.lights[i].shadow_map_res_x;
                            const float dy0 = float(offset_y) / dir_light_buf.lights[i].shadow_map_res_y;
                            const float dy1 = float(offset_y + 1) / dir_light_buf.lights[i].shadow_map_res_y;

                            shadow_factors[0] = texture(dir_shadow_maps[dir_light_buf.lights[i].shadow_map_id], vec3(shadow_map_texcoords.x + dx0, shadow_map_texcoords.y + dy0, sm_id)).x;
                            shadow_factors[1] = texture(dir_shadow_maps[dir_light_buf.lights[i].shadow_map_id], vec3(shadow_map_texcoords.x + dx1, shadow_map_texcoords.y + dy0, sm_id)).x;
                            shadow_factors[2] = texture(dir_shadow_maps[dir_light_buf.lights[i].shadow_map_id], vec3(shadow_map_texcoords.x + dx0, shadow_map_texcoords.y + dy1, sm_id)).x;
                            shadow_factors[3] = texture(dir_shadow_maps[dir_light_buf.lights[i].shadow_map_id], vec3(shadow_map_texcoords.x + dx1, shadow_map_texcoords.y + dy1, sm_id)).x;

                            for(uint sf = 0; sf < 4; sf++)
                            {
                                shadow_factors[sf] = shadow_factors[sf] < shadow_map_texcoords.z ? 0.0f : 1.0f;
                            }

                            shadow_factor += mix(mix(shadow_factors[0], shadow_factors[1], t.x), mix(shadow_factors[2], shadow_factors[3], t.x), t.y);
                        }
                    }
