...
void main()
{
    const float xd = distance(world_pos_in, common_buf.cur_pos_terrain);

    if(common_buf.cur_terrain_intersection != 0)
    {
        if(xd >= common_buf.editor_terrain_tool_inner_radius && xd <= common_buf.editor_terrain_tool_outer_radius)
        {
